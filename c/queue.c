...
struct PriorityQueue * new_priority_queue(int max_elements) {
    ...
    pq->previous = (int*)calloc(max_elements, sizeof (int));
    pq->first = -1;
    pq->next_free = 0;
    pq->size = 0;
    return pq;
...
void priority_queue_add_with_priority(struct PriorityQueue * q, unsigned int elem, unsigned int priority) {
    ...
    q->previous[element_index] = element_index - 1;
}
...
unsigned int priority_queue_extract_min(struct PriorityQueue * q) {
    ...
    if (index == 0) {
        ...
        q->elements[q->previous[index]].next = q->elements[index].next;
        if (q->elements[index].next != -1)
            q->previous[q->elements[index].next] = q->previous[index];
    }
    q->previous[index] = 0;
    q->size--;
    return val;
